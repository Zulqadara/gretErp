...
if(isset($_GET['approve'])){
	...
	$db->query("UPDATE `eassignment` SET hodstatus='approved' WHERE eassignmentid='$edit_id'");	
	header('Location: eassignment.php');
}
if(isset($_GET['reject'])){
	$edit_id = (int)$_GET['reject'];
	$db->query("UPDATE `eassignment` SET hodstatus='rejected' WHERE eassignmentid='$edit_id'");	
	header('Location: eassignment.php');
}
?> 
  <body>
...
<br>
			...
			$sql = "select * from `eassignment` inner join staff on staff.staffid = `eassignment`.staffid where status='pending' AND hodstatus='pending' AND hrstatus='pending'";
