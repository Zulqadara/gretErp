...
if(isset($_GET['approve'])){
	...
	$db->query("UPDATE `eassignment` SET hrstatus='approved' WHERE eassignmentid='$edit_id'");	
	header('Location: eassignment.php');
}
if(isset($_GET['reject'])){
	$edit_id = (int)$_GET['reject'];
	$db->query("UPDATE `eassignment` SET hrstatus='rejected' WHERE eassignmentid='$edit_id'");	
	header('Location: eassignment.php');
}
?> 
  <body>
...
inner join staff on staff.staffid = `eassignment`.staffid
			where status='pending' AND hodstatus='approved' AND hrstatus='pending'";
