...
$pdf->Cell(115,10,'Lecturers List: ', 0, 1, 0);
...
$pdf->Cell($w, 10, "Lecturer ID", 1, 0, "L", 1); 
$pdf->Cell(30, 10, "Lecturer Name", 1, 0, "L", 1); 
$pdf->Cell($w, 10, "Lecturer Courses", 1, 0, "L", 1); 
...
$pdf->setXY($x2, $y);
$pdf->setFont("Arial","B","9");
$query_result = "select (staff.staffid) as c,(staff.name) as d, GROUP_CONCAT(courses.name SEPARATOR '"."\r\n| "."') AS courses
	from staff
...
$x=$pdf->getx();
$pdf->myCell(30,$h,$x,$row['d']);
$x=$pdf->getx();
$pdf->myCell($w,$h,$x,$row['courses']);
$pdf->Ln();
$y += 8;
		$count++;
		        if ($y > 170)   
