...
if(isset($_GET['approve'])){
	...
	$db->query("UPDATE `offday` SET status='approved' WHERE offdayid='$edit_id'");	
	header('Location: off.php');
}
if(isset($_GET['reject'])){
	$edit_id = (int)$_GET['reject'];
	$db->query("UPDATE `offday` SET status='rejected' WHERE offdayid='$edit_id'");	
	header('Location: off.php');
}
?> 
  <body>
...
<br>
			...
			$sql = "select * from `offday` 
			inner join staff on staff.staffid = `offday`.staffid
